...
[TestFixture]
public class ReportedAgencyConverterTests
{
    [Test]
    public async Task TestConvert()
    {
        ...
        IList<DBKeyImportLog> logs = LocalDataHelper.GetReportedAgencyImportLogData();
        ...
        {
            IList<ReportedAgency>? reportedAgencies = new ReportedAgencyConverter().Convert(importAuditPackage);
            Assert.NotNull(reportedAgencies);
            Assert.True(reportedAgencies.Any());
        }
