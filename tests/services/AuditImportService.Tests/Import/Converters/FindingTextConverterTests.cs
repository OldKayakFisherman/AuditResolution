...
public class FindingTextConverterTests
{
    ...
    public async Task TestConvert()
    {
        ...
        IList<DBKeyImportLog> logs = LocalDataHelper.GetFindingTextDbKeyImportLogData();
        ...
        {
            IList<FindingText>? findingTexts = new FindingTextConverter().Convert(importAuditPackage);
            Assert.NotNull(findingTexts);
            Assert.True(findingTexts.Any());
        }
